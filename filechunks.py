...
def _hash_file(basedir: str, relpath: str, file_progress_func=None) -> List[FileChunk]:
    '''
    ...
    with open(path, 'rb') as f:
        while pos < st.st_size:
            ...
            res.append(FileChunk(
                    ...
                    sha1=hashlib.sha1(f.read(sz)).hexdigest(),
                    urls=[],
                    has_chunk=True))
            pos += sz
            if file_progress_func:
...
def _hash_dir(basedir: str, old_chunks=None, progress_func=None) -> List[FileChunk]:
    '''
    ...
    def file_needs_rehash(filename: str):
        try:
            c = fn_to_old_chunk[filename]
            s = os.stat(os.path.join(basedir, filename))
    ...
    files_needing_rehash = set([fn for fn in fnames if file_needs_rehash(fn)])
    if len(fnames) == len(fn_to_old_chunk.keys()) and len(files_needing_rehash) == 0:
        return old_chunks

    # Prepare progress reporting
    total_size = int(sum([os.stat(os.path.join(basedir, f)).st_size for f in fnames]))
    ...
    for fn in fnames:
        if fn not in files_needing_rehash:
            ...
            res.extend(_hash_file(basedir, fn, file_progress_func=file_progress))
...
async def monitor_folder_forever(basedir: str, update_interval: float, progress_func=None):
    ...
    chunks = []
    while True:
        new_chunks = _hash_dir(basedir, chunks, progress_func=progress_func)
